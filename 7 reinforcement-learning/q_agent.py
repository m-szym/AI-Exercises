...
class QAgent(Agent):
    ...
    def __init__(self, n_states, n_actions,
        ...
        self.epsilon = 0.3           # epsilon (p-wo akcji losowej)
        self.eps_decrement = 0.005     # wartość, o którą zmniejsza się epsilon po każdym kroku
    ...
    def choose_action(self, state: State) -> Action:
        ...
        if r > self.epsilon:
            ...
            action = np.random.randint(low=0, high=len(self.action_space))
    ...
    def learn(self, state: State, action: Action, reward: float, new_state: State, done: bool) -> None:
        ...
        #if not done:
        p = np.max(self.q_table[new_state])
        q = np.max(self.q_table[new_state, :])
