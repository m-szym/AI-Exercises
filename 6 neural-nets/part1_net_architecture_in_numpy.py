...
def zad1_single_neuron(student_id):
    ...
    # TOD: ustawienie właściwych wag (0.5 point)
    w1 = -1.0
    w2 = -2.0
    b1 = 0.5
...
def zad2_two_layer_net(student_id):
    ...
    # TOD: warstwy mozna składać w wiekszy model
    class SimpleTwoLayerNetwork:
        ...
        def forward(self, x_data):
            hidden_activations = self.hidden_layer.forward(x_data)
            return self.output_layer.forward(hidden_activations)

    # model zainicjowany losowymi wagami
    model = SimpleTwoLayerNetwork(n_in=n_features, n_hidden=2, n_out=1)

    # TOD: ustawienie właściwych wag
    model.hidden_layer.W[:, 0] = 2.0, -1.0    # wagi neuronu h1
    model.hidden_layer.W[:, 1] = -1.0, 0.3     # wagi neuronu h2
    model.hidden_layer.b[:] = -1.0, 0.5         # biasy neuronów h1 i h2
...
if __name__ == '__main__':
    ...
    student_id = 193229         # Twój numer indeksu, np. 102247
